...
export default function Dashboard() {
  ...
  function openEditor(value: Editor) { setMessage(""); setEditor(value); }
  ...
  async function send(command: FinanceCommand) {
    ...
    try {
      ...
      if (!response.ok) {
        if ([400, 409, 413].includes(response.status)) { setPending(null); sessionStorage.removeItem(pendingKey); setStale(true); }
  ...
  return <main className="finance">
    ...
    {!data ? <section className="notice"><h2>Laporan belum tersedia</h2><p>{busy ? "Sedang memuat catatan…" : "Tekan Muat ulang untuk mencoba kembali."}</p></section> : <>
      ...
      {tab === "Cocokkan" && <>
        ...
        <section><h2>Bukti provider</h2>{data.providers.map(p => <article className="finance-row" key={p.transaction_id}><div><strong>{p.transaction_id}</strong><small>{p.merchant_id} · {stamp(p.paid_at)} WIB</small><b>{money(p.amount)}</b><small>{p.match ? `Terhubung: ${p.match.device_id} / ${p.match.order_id} · selisih ${money(p.match.difference)}` : "Belum berpasangan"}</small></div>{p.match ? <button className="secondary" disabled={locked} onClick={() => openEditor({ action: "unmatch", proof: p })}>Lepas pasangan</button> : <button className="secondary" disabled={locked} onClick={() => setProof(p)}>{proof?.transaction_id === p.transaction_id ? "Dipilih" : "Pilih bukti"}</button>}</article>)}{!data.providers.length && <p>Belum ada bukti provider pada periode ini.</p>}{pager("providers", data.counts.providers)}</section></div>
      </>}
      {tab === "Biaya" && <>
        <section className="finance-selection"><div><strong>Tarif estimasi berdasarkan masa berlaku</strong><p>Tarif diisi dari perjanjian merchant. Angka estimasi disimpan sebagai snapshot dan dipisahkan dari biaya aktual.</p></div><button disabled={locked} onClick={() => openEditor({ action: "fee_profile" })}>Tambah tarif</button></section>
