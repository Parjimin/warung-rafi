...
export default function Catalog() {
  ...
  return <main><div className="heading"><div><p className="eyebrow">MENU WARUNG</p><h1>Siap disajikan hari ini.</h1><p>Atur pilihan makanan, harga, dan ketersediaan untuk kasir.</p></div><button className="secondary" onClick={async () => { if (dirty && !confirm("Ada perubahan belum disimpan. Tetap keluar?")) return; await fetch("/api/auth/logout", { method: "POST" }); location.assign("/login"); }}>Keluar</button></div>
    ...
    {editing && <dialog ref={editor} aria-labelledby="edit-title" className="editor" onCancel={event => { event.preventDefault(); if (!uploading) setEditing(null); }}><h2 id="edit-title">Detail menu</h2><form onSubmit={event => { event.preventDefault(); setItems(previous => previous.some(item => item.id === editing.id) ? previous.map(item => item.id === editing.id ? editing : item) : [...previous, editing]); setDirty(true); setEditing(null); }}><fieldset disabled={uploading}><div className="editor-fields">
      <label>Nama menu<input autoFocus value={editing.name} maxLength={60} required onChange={event => setEditing({ ...editing, name: event.target.value })} /></label><div className="form-row"><label>Kategori<select aria-label="Kategori" value={editing.category} onChange={event => setEditing({ ...editing, category: event.target.value })}>{categories.map(value => <option key={value}>{value}</option>)}</select></label><label>Harga (rupiah)<input type="number" min="1" max="1000000000" step="1" required value={editing.price || ""} onChange={event => setEditing({ ...editing, price: Number(event.target.value) })} /></label></div><div className="photo-edit">{editing.imageUrl && <img src={editing.imageUrl} alt="Pratinjau foto menu" referrerPolicy="no-referrer" />}<label>Unggah foto<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { void upload(event.target.files?.[0]); event.target.value = ""; }} /></label><small>JPG, PNG, WebP · maksimal 3 MB. Foto tampil di kasir setelah draf diterbitkan.</small>{editing.imageUrl && <button type="button" className="secondary" onClick={() => setEditing({ ...editing, imageUrl: null })}>Lepas foto dari menu</button>}</div><p role="status">{uploading ? "Mengunggah foto…" : photoError}</p><details><summary>Gunakan alamat foto</summary><label>Alamat foto HTTPS (opsional)<input type="url" value={editing.imageUrl ?? ""} onChange={event => setEditing({ ...editing, imageUrl: event.target.value || null })} /></label></details><label className="check"><input type="checkbox" checked={editing.available} onChange={event => setEditing({ ...editing, available: event.target.checked })} /> Tersedia untuk dijual</label></div><div className="form-row editor-actions"><button type="button" className="secondary" onClick={() => setEditing(null)}>Batal</button><button>Terapkan ke draf</button></div></fieldset></form></dialog>}
  </main>;
}
